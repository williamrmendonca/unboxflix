...
export default function ListMovies({ title, genres }) {
    ...
    useEffect(() => {
        ...
        axios.get(`https://api.themoviedb.org/3/discover/tv?api_key=${API_KEY}&language=${localeLanguageTag}&sort_by=popularity.desc&with_genres=${genres}`)
            .then(res => {
                setData(res.data);
        ...
        return (
            <View>
                <View style={{flexDirection: 'row', justifyContent: 'space-between', alignItems: 'baseline', marginTop: 8, marginBottom: 8}}>
                    <Text style={{color: '#fff', fontSize: 24, fontWeight: 'bold'}}>Séries {title}</Text>
                </View>
                <FlatList
